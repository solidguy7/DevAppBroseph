...
@post_router.post('/{id}')
async def create_post(id: UUID, post: PostIn, user: str = Depends(authenticate), session: AsyncSession = Depends(get_session)) -> dict:
...
async def like(id: UUID, user: str = Depends(authenticate), session: AsyncSession = Depends(get_session)) -> dict:
    user_id = await session.execute(select(User.id).where(User.username == user))
